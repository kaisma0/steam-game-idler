...
pub async fn get_games_list(
    ...
    let games_list = read_games_list_from_temp()?;
...
// Helper: Gets the temp directory for the fetcher output
fn get_fetcher_temp_dir() -> Result<std::path::PathBuf, String> {
    let mut temp = std::env::temp_dir();
    temp.push("steam-game-idler");
    if !temp.exists() {
...
// Helper: Reads games_list.json from the temp directory
fn read_games_list_from_temp() -> Result<Vec<GameData>, String> {
    let path = get_fetcher_temp_dir()?.join("games_list.json");
...
async fn run_steam_game_fetcher() -> Result<(), String> {
    ...
    let temp_dir = get_fetcher_temp_dir()?;
    let json_path = temp_dir.join("games_list.json");
...
pub fn get_games_list_cache(
    ...
    let games_list = {
        ...
        if games_file_path.exists() {
            ...
            match read_games_list_from_temp() {
                Ok(games) => json!(games),
                Err(_) => json!([]),
            }
        }
